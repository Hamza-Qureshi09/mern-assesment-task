...
const LoginPage: React.FunctionComponent<ILoginPageProps> = (props) => {
    ...
    const signinUser = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!email || !password) {
            toast.error("please fill all the fields first!", { position: 'top-center', duration: 2000 })
            return
        }
        try {
            setsubmitLoading(true)
            const request = await LoginRoute({
            ...
            if (request?.status === 200) {
                ...
                router.replace('/')
                toast.success("Successfully Login", { duration: 3000, position: 'top-center' })
                return;
            }
    ...
    const ForgetPassword = () => {
        toast.error("Sorry! This Feature is not Ready yet!", { duration: 2500, position: 'top-center' })
        return;
    }
    return <>
                                ...
                                <div className="flex flex-col w-full max-w-full px-3 mx-auto lg:mx-0 shrink-0 md:flex-0 md:w-7/12 lg:w-5/12 xl:w-4/12">
                                    <div className="relative flex flex-col min-w-0 break-words bg-transparent border-0 shadow-none lg:py-4 rounded-2xl bg-clip-border">
                                        <div className="p-6 pb-0 mb-0">
                                            <h4 className="font-bold font-Merriweather text-xl text-gray-800">Sign In</h4>
                                            <p className="mb-0 font-Lato">Enter your email and password to sign in</p>
                                        </div>
                                            ...
                                            <form method="POST" onSubmit={signinUser}>
                                                <div className="mb-4">
                                                    <input onChange={handleInput}
                                                        value={inputs.email}
                                                        name="email" type="email" placeholder="Email" required className="focus:shadow-primary-outline text-sm leading-5 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding p-3 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-fuchsia-300 focus:outline-none" />
                                                </div>
                                                <div className="mb-4">
                                                    <input onChange={handleInput}
                                                        value={inputs.password}
                                                        id="password" name="password" placeholder="Enter Password" type="password" required className="focus:shadow-primary-outline text-sm leading-5 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding p-3 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-fuchsia-300 focus:outline-none" />
                                                </div>
                                                <div className="flex items-center justify-between">
                                                    <div className="text-sm cursor-pointer" onClick={ForgetPassword}>
                                                        <a className=" sm:text-blue-500 text-blue-700 hover:text-blue-600 font-Poppins text-xs underline underline-offset-8">Forgot your password?</a>
                                                    </div>
                                                </div>
                                        ...
                                        <div className="border-black/12.5 rounded-b-2xl border-t-0 border-solid p-6 text-center pt-0 px-1 sm:px-6">
                                            <p className="mt-4 mb-0 leading-normal text-sm">Don't have an account? <Link href="/register" className="font-bold text-slate-700">Sign up</Link></p>
                                        </div>
                                    </div>
                                </div>
